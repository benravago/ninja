...
final class TypeEvaluator {
    ...
    private Object evaluateSafely(Expression expr) {
        ...
        if (expr instanceof AccessNode) {
            var accessNode = (AccessNode)expr;
    ...
    private Type getEvaluatedType(Optimistic expr) {
        if (expr instanceof IdentNode) {
            ...
            // We could do better, but these are all heuristics and we can gradually introduce them as needed.
            // An easy one would be to do the same for .call(this) idiom.
            var callExpr = (CallNode)expr;
                ...
                if (callExpr.getArgs().isEmpty()) {
                    ...
                    if (data != null) {
                        ...
                        if (returnType == Type.BOOLEAN) {
                            // We don't have optimistic booleans. In fact, optimistic call sites getting back boolean currently deoptimize all the way to Object.
                            return Type.OBJECT;
                        }

 ...
 *
 * This object acts as the API entry point to Java platform specific functionality, dealing with creating new instances of Java classes, subclassing Java classes, implementing Java interfaces, converting between Java arrays and ECMAScript arrays, and so forth.
 */
@ScriptClass("Java")
public final class NativeJava {
     ...
     * </pre>
     * <p>
     * In the special case of inner classes, you can either use the JVM fully qualified name, meaning using {@code $} sign in the class name, or you can use the dot:
     * <pre>
     ...
     * If method names are overloaded, the JavaScript function will provide implementation for all overloads.
     * <p>
     * E.g.:
     * <pre>
     ...
     * </pre>
     * <p>
     * 'objTypeName' is the object whose JS string value represents the type name.
     * You can use names of primitive Java types to obtain representations of them, and you can use trailing square brackets to represent Java array types.
     */
    @Function(attributes = Attribute.NOT_ENUMERABLE, where = Where.CONSTRUCTOR)
     ...
     * </pre>
     * <p>
     * 'objType' is either a {@link #type(Object, Object) type object} or a String describing the type of the Java object to create.
     * Can not be null.
     * If undefined, a "default" conversion is presumed (allowing the argument to be omitted).
     * <p>
     * Returns a Java object whose value corresponds to the original script object's value.
     * Specifically, for array target types, returns a Java array of the same type with contents converted to the array's component type.
     * Converts recursively when the target type is multidimensional array.
     * For {@link List}, {@link Deque}, {@link Queue}, or {@link Collection}, returns a live wrapper around the object, see {@link ListAdapter} for details.
     ...
     * </pre>
     * <p>
     * 'objArray' is the java array or collection. Can be null.
     * <p>
     * Returns a JavaScript array with the copy of Java array's or collection's contents.
     * Returns null if objArray is null.
     */
    @Function(attributes = Attribute.NOT_ENUMERABLE, where = Where.CONSTRUCTOR)
    ...
    public static List<String> getProperties(Object object) {
        if (object instanceof StaticClass) {
            ...
            var clazz = ((StaticClass)object).getRepresentedClass();
     ...
     * <ul>
     * <li>
     * Every specified list of Java types will have one extender subclass in Nashorn per caller protection domain - repeated invocations of {@code extend} for the same list of types for scripts same protection domain will yield the same extender type.
     ...
     * <li>
     * It is also possible to specify an ordinary JavaScript object as the last argument to {@code extend}.
     * In that case, it is treated as a class-level override.
     * {@code extend} will return an extender class where all instances will have the methods implemented by functions on that object, just as if that object were passed as the last argument to their constructor.
     * Example:
     * <pre>
     ...
     * <p>
     * Returns a new {@link StaticClass} that represents the adapter for the original types.
     */
    @Function(attributes = Attribute.NOT_ENUMERABLE, where = Where.CONSTRUCTOR)

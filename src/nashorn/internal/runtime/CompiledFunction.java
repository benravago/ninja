...
final class CompiledFunction {
    ...
    CompiledFunction(MethodHandle invoker, MethodHandle constructor, int flags, MethodType callSiteType, Specialization specialization, DebugLogger log) {
        ...
        if (specialization != null && specialization.isOptimistic()) {
             ...
             * In order to fit in with the relinking framework, the current thinking is that the methods still take a program point to fit in with other optimistic functions, but* it is set to "first", which is the beginning of the method.
             * The relinker can tell the difference between builtin and JavaScript functions.
             * This might change.
             * TODO:
             */
    ...
    private static MethodHandle createConstructorFromInvoker(MethodHandle invoker) {
        ...
        if (needsCallee) {
            ...
            // so (this, callee, args...) x (callee) => (callee, args...),
            // or...
    ...
    /**
     * Returns an invoker method handle for this function when invoked as a constructor.
     * Note that the handle is safely composable in the sense that you can compose it with other handles using any combinators even if you can't affect call site invalidation. If this compiled function is non-optimistic, then it returns the same value as {@link #getConstructor()}.
     * However, if the function is optimistic, then this handle will incur an overhead as it will add an intermediate internal call site that can relink itself when the function needs to regenerate its code to always point at the latest generated code version.
    ...
    private static void relinkComposableInvoker(CallSite cs, CompiledFunction inv, boolean constructor) {
        var handleAndAssumptions = inv.getValidOptimisticInvocation(new Supplier<MethodHandle>() {
            @Override
     ...
     * As such, it will probably not be used as a final guarded invocation, but rather as a holder for an invocation handle and switch point to be decomposed and reassembled into a different final invocation by the user of this method.
     * Any recompositions should take care to continue to use the switch point.
     * If that is not possible, use {@link #createComposableConstructor()} instead.
    ...
    private static List<String> toStringInvalidations(Map<Integer, Type> ipp) {
        ...
        for (var iter = ipp.entrySet().iterator(); iter.hasNext(); ) {
            ...
            type = switch (entry.getValue().getBytecodeStackType()) {
                case 'A' -> "object";
                case 'I' -> "int";
                case 'J' -> "long";
                case 'D' -> "double";
    ...
    private static class OptimismInfo {
        ...
        boolean requestRecompile(RewriteException e) {
            ...
            if (previousFailedType != null && !previousFailedType.narrowerThan(retType)) {
                var stack = e.getStackTrace(); // TODO: use StackWalker#walk

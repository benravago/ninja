...
final class SetMethodCreator {
    ...
    private class SetMethod {
        ...
        GuardedInvocation createGuardedInvocation() {
            ...
            return new GuardedInvocation(methodHandle,
                NashornGuards.getGuard(sobj, property, desc, explicitInstanceOfCheck),
                (SwitchPoint)null,
    ...
    private SetMethod createExistingPropertySetter() {
        ...
        if (NashornCallSiteDescriptor.isDeclaration(desc) && property.needsDeclaration()) {
            ...
            // The map is installed with a fast compare-and-set method if the pre-callsite map is stable (which should be the case for function scopes).
    ...
    private SetMethod createNewSetter(Property property, SwitchPoint builtinSwitchPoint) {
        ...
        // The setter goes to "nop" VOID_RETURN if false or throws an exception
        var extCheck = MH.insertArguments(ScriptObject.EXTENSION_CHECK, 1, name);

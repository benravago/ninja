...
final class NashornPrimitiveLinker implements TypeBasedGuardingDynamicLinker, GuardingTypeConverterFactory, ConversionComparator {
     ...
     * Returns one of {@link jdk.dynalink.linker.ConversionComparator.Comparison} values signifying which target type should be favored for conversion.
     */
    @Override
    public Comparison compareConversion(Class<?> sourceType, Class<?> targetType1, Class<?> targetType2) {
        ...
        if (Number.class.isAssignableFrom(sourceType)) {
            ...
            // If exactly one of the targets is a character, pick it. Numbers can be reasonably converted to chars using the UTF-16 values.
        ...
        if (sourceType == String.class || sourceType == Boolean.class || Number.class.isAssignableFrom(sourceType)) {
            ...
            // Basically, choose the widest possible primitive type. (First "if" returning TYPE_2_BETTER is correct; when faced with a choice between double and int, choose double).

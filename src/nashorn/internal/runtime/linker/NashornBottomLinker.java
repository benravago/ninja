...
final class NashornBottomLinker implements GuardingDynamicLinker, GuardingTypeConverterFactory {
    ...
    private static GuardedInvocation linkBean(LinkRequest linkRequest) throws Exception {
        ...
        switch (NashornCallSiteDescriptor.getStandardOperation(desc)) {
            ...
            default -> {
                // Everything else is supposed to have been already handled by Bootstrap.beansLinker delegating to linkNoSuchBeanMember
    ...
    private static MethodHandle adaptThrower(MethodHandle handle, CallSiteDescriptor desc) {
        ...
        return MethodHandles.dropArguments(handle, paramCount, targetType.parameterList().subList(paramCount, targetType.parameterCount()))
                            .asType(targetType);
    }

    @SuppressWarnings("unused")
    private static void throwPropertySetter(Object self, Object name) {
        throw createTypeError(self, name, "cant.set.property");
    }

    @SuppressWarnings("unused")
    private static boolean throwPropertyRemover(Object self, Object name) {
    ...
    private static boolean isNonConfigurableProperty(Object self, Object name) {
        if (self instanceof StaticClass) {
            var clazz = ((StaticClass)self).getRepresentedClass();
    ...
    private static final Map<Class<?>, MethodHandle> CONVERTERS = Map.of(
        boolean.class, JSType.TO_BOOLEAN.methodHandle(),
        double.class, JSType.TO_NUMBER.methodHandle(),
        int.class, JSType.TO_INTEGER.methodHandle(),
        long.class, JSType.TO_LONG.methodHandle(),
        String.class, JSType.TO_STRING.methodHandle()
    );

 ...
 *
 * The reason we have our own subclass is that we're storing flags in an additional primitive field. The class also exposes some useful utilities in form of static methods.
 */
public final class NashornCallSiteDescriptor extends CallSiteDescriptor {
     ...
     * Equivalent to {@code ((NamedOperation)desc.getOperation()).getName().toString()} for descriptors with a named operand. For descriptors without named operands returns null.
     ...
     * @throws ClassCastException if the base operation is not a standard operation. This method is only safe to use when the base operation is known to be a standard operation (e.g. all Nashorn call sites are such, so it's safe to use from internal linkers).
     */
